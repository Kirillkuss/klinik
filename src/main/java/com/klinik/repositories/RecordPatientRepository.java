...
public interface RecordPatientRepository extends JpaRepository<RecordPatient,Long> {
    ...
    @Query( value = "SELECT r.* FROM Record_patient r left join Card_patient c on c.id_card_patient = card_patient_id left join Patient p on p.id_patient = c.patient_id \n" 
                  + " where p.id_patient = ?1 and (r.date_record BETWEEN ?2 and ?3 )", nativeQuery = true  )
