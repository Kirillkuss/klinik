...
function lazyPatients( page, size) {
    ...
    $.getJSON(protocol + '//'+ hostname + ':' + port +'/web/patients/list/{page}{size}?page='+page+'&size='+size, function(json) {
        var tr=[];
...
function listPatient() {
    ...
    $.getJSON(protocol + '//'+ hostname + ':' + port +'/web/patients/all', function(json) {
        var tr=[];
        for (var i = 0; i < json.length; i++) {
            tr.push('<tr>');
            tr.push('<td>' + json[i].idPatient + '</td>');
    ...
    $(document.getElementById("findByPatient")).on( "click",function(){
        ...
        if(  word.length  == 0 ){
            ...
            $.ajax({
                type: "GET",
                contentType: "application/json; charset=utf-8",
                url: protocol + '//'+ hostname + ':' + port +"/web/patients/find/{word}",
                data:{ word: word } ,
                cache: false,
                success: function( json ) {
                    var tr=[];
